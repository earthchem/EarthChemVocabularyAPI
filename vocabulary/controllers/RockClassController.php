...
class RockclassController extends RESTController
{
    ...
    public function getAction($request) {
        ...
        if(isset($request->url_elements[2])) {
			...
			if(is_int($searchid)){
				$row = $this->db->get_row("select * from taxonomic_classifier where taxonomic_classifier_type_cv='Rock Class' and taxonomic_classifier_num=$searchid and status = 1");
        	...
        	if($_GET){
        		...
        		if($_GET['label']){
					...
					$rows = $this->db->get_results("select * from taxonomic_classifier where taxonomic_classifier_type_cv='Rock Class' and lower(taxonomic_classifier_name) like '%$label%' and status = 1
				...
				$rows = $this->db->get_results("select * from taxonomic_classifier where taxonomic_classifier_type_cv='Rock Class' and status = 1 order by taxonomic_classifier_name");
				$data['resultcount']=count($rows);
				foreach($rows as $row){
