...
class RockclassController extends RESTController
{
    ...
    public function getAction($request) {
        ...
        if(isset($request->url_elements[2])) {
			...
			if(is_int($searchid)){
				$row = $this->db->get_row("select * from earthchem.taxonomic_classifier where taxonomic_classifier_type_cv='Rock Class' and taxonomic_classifier_num=$searchid and status = 1");
				if($row->taxonomic_classifier_num){
						...
						$data=$this->jsonObject("rockClass", $num, $preflabel, $altlabel, $definition, "rcla");
				}else{
        	...
        	if($_GET){
        		...
        		if($_GET['label']){
					...
					$rows = $this->db->get_results("select * from earthchem.taxonomic_classifier where taxonomic_classifier_type_cv='Rock Class' and lower(taxonomic_classifier_name) like '%$label%' and status = 1
					...
					if(count($rows) > 0){
						...
						foreach($rows as $row){
							...
							$data['results'][]=$this->jsonObject("rockClass", $num, $preflabel, $altlabel, $definition, "rcla");
				...
				$rows = $this->db->get_results("select * from earthchem.taxonomic_classifier where taxonomic_classifier_type_cv='Rock Class' and status = 1 order by taxonomic_classifier_name");
				$data['resultcount']=count($rows);
				foreach($rows as $row){
        ...
        if(isset($request->url_elements[2])) {
			...
			if(is_int($searchid) && $searchid!=0){
				...
				if($row->taxonomic_classifier_num){
					...
					header("Not Found", true, 404);
					$data["Error"] = "Rock Class $id not found.";
				}
			}else{
				header("Not Found", true, 404);
				$data["Error"] = "Rock Class $id not found.";
    ...
    public function postAction($request) {
        ...
        if(isset($request->url_elements[2])) {
			...
			if($preflabel == ""){
				...
				$returnuri = $this->baseUri."rockClass/".$returnpkey;
				$data=$p;
				$data['uri']=$returnuri;
